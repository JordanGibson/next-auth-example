import {build_status, Prisma, PrismaClient, PrismaPromise} from "@prisma/client";
import type { build, suite, comment } from "@prisma/client";
...
import StateEnum = Build.StateEnum;
import {createLocator} from "./createLocator.js";
import prisma from "./prisma/client.js";
import * as async from "async";
import {signOut} from "next-auth/react";
import type {TestOccurrence} from "../../../../Downloads/ts-fetch-client/api.js";
...
async function getAndStoreBuildResultsSummary(builds: Prisma.buildCreateManyInput[]) {
    const buildResultsSummary: Prisma.build_results_summaryCreateManyInput[] = (
        await Promise.all(
            builds.map(async (build) => {
                const testOccurrencesResponse = await testOccurrenceApi.getAllTestOccurrences(
                    createLocator({
                        build: {
                ...
                await prisma.test.createMany({
                    data: testOccurrences.map((testOccurrence): Prisma.testCreateManyInput => ({
                ...
                await prisma.test_occurrence.createMany({
                    data: testOccurrences.map((testOccurrence): Prisma.test_occurrenceCreateManyInput => ({
...
type IndexedSuite = { builds: Prisma.buildCreateManyInput[]; resultSummary: Prisma.build_results_summaryCreateManyInput[] };
...
let indexedSuites: IndexedSuite[] = await async.mapLimit(suitesToIndex, 5, indexSuite);
